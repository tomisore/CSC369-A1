...
{  
   // still not sure if i should use spin locks
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
    ...
    if (syscall < 0  || syscall > NR_syscall || syscall == MY_CUSTOM_SYSCALL){
	return -EINVAL;
    }
	...
	case REQUEST_START_MONITORING:
		my_sys_strtmon(syscall, pid);
		break;
	case REQUEST_STOP_MONITORING:
		my_sys_stopmon(syscall, pid);
		break:
	default:
		return -EINVAL;	
...
/** my_syscall helper function
 *
 */
asmlinkage long my_sys_intc(int syscall){
    //check if the root
    if( current_uid() != 0){
	return -EPERM;
    }
    ...
    // save orig syscall and mark for interception.
    spin_lock(&my_table_lock);
    table[syscall].f = sys_call_table[syscall];
    table[syscall].intercepted = 1;
    spin_unlock(&my_table_unlock);
...
*
*/
asmlinkage long my_sys_rels(int syscall){
    //check if the root
    if( current_uid() != 0){
       return -EPERM;
    }
...
*
*/
asmlinkage long my_sys_strtmon(int syscall, int pid){
     if (current_uid() != 0){
           // permission checks 
	   if (pid = 0 ){
		return -EPERM;
	...
	if(table[syscall].monitored != 2){	
	     ...
	     // add all pid to syscall list, not sure if i added all pids right way
	     struct task_struct *task;
	     for_each_process(task){{
	        add_pid_sysc(task->pid, syscall);	     
	     }	
	     //set .monitored value 
...
*
*/
asmlinkage long my_sys_stopmon(int syscall, int pid){
	if (current_uid() != 0){
	// permission checks 
              if (pid = 0 ){
		   return -EPERM;
	...
	if (pid == 0){
	     ...
	     // check if pid is actually being monitored
...
static int init_function(void) {
     ...
     //Hijack the exit group sys_call
     orig_exit_group = sys_call_table[__NR_exit_group];
     sys_call_table[__NR_exit_group] = &my_exit_group;

     // Hijack my custom syscall
...
{        
    spin_lock(&sys_call_table_lock);
    set_addr_rw((long unsigned int) sys_call_table);
    //Restore custom syscall
    sys_call_table[MY_CUSTOM_SYSCALL] = ori_custom_syscall;
    sys_call_table[__NR_exit_group] = orig_exit_group;
    // restore intercpted syscall 
    spin_lock(&my_table_lock);
    for( int s = 0 ;s <NR_syscalls; s++){
	    sys_call_table[s] = table[s].f;//not sure about this.
	    table[s].intercepted = 0;
            destroy_list(s);
    } 
    set_addr_ro((long unsigned int) sys_call_table);
